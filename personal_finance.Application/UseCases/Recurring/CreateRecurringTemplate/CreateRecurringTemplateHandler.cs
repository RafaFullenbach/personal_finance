...
namespace personal_finance.Application.UseCases.Recurring.CreateRecurringTemplate
{
    public sealed class CreateRecurringTemplateHandler
    {
        ...
        public async Task<CreateRecurringTemplateResult> HandleAsync(CreateRecurringTemplateCommand command)
        {
            ...
            if (account is null) throw NotFoundException.For("Conta", command.AccountId);
            ...
            if (category is null) throw NotFoundException.For("Categoria", command.CategoryId);

            if (!category.IsActive)
                throw new BusinessRuleException("A categoria está desativada. Não é possível criar modelos recorrentes.");

            if (!account.IsActive)
                throw new BusinessRuleException("A conta está desativada. Não é possível criar modelos recorrentes.");
