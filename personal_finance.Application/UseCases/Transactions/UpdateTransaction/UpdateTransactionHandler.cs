...
namespace personal_finance.Application.UseCases.Transactions.UpdateTransaction
{
    public sealed class UpdateTransactionHandler
    {
        ...
        {
            if (command.Amount <= 0)
                throw ValidationException.Invalid("Valor deve ser maior que zero.", ErrorCodes.TransactionInvalidAmount);

            if (command.CompetenceMonth < 1 || command.CompetenceMonth > 12)
                throw ValidationException.Invalid("Mês de competência deve estar entre 1 e 12.", ErrorCodes.TransactionInvalidCompetence);

            if (command.CompetenceYear < 2000 || command.CompetenceYear > 2100)
                throw ValidationException.Invalid("Ano de competência está fora da faixa.", ErrorCodes.TransactionInvalidCompetence);

            if (!Enum.TryParse<TransactionType>(command.Type, true, out var type))
                throw ValidationException.Invalid($"Tipo de lançamento '{command.Type}' é inválido.", ErrorCodes.TransactionInvalidType);

            var tx = await _transactions.GetByIdAsync(command.Id);
            if (tx is null)
                throw NotFoundException.For("Lançamento", command.Id);
            ...
            if (command.CategoryId.HasValue)
            {
                ...
                if (!category.IsActive)
                    throw new BusinessRuleException("Categoria desativada. Não é possível atribuir transações a essa categoria.");
            }
